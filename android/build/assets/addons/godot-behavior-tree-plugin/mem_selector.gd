...
func open(tick: Tick) -> void:
	...
	tick.blackboard.set_value("runningChild", 0, tick.tree, self)


func tick(tick: Tick) -> int:
	
	var result := OK #if we have no children, assume success
	var childIdx = tick.blackboard.get_value("runningChild", tick.tree, self) #get child index
	...
	for idx in range(childIdx, get_child_count()):
		...
		if result != FAILED:
			if result == ERR_BUSY:
				tick.blackboard.set_value("runningChild", idx, tick.tree, self)
			break
